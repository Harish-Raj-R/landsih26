...
public class AdminController {
    ...
    @GetMapping("/api/supervisor/stats")
    @PreAuthorize("hasAnyRole('DEPARTMENT_SUPERVISOR', 'ADMIN')")
    public ResponseEntity<SupervisorDashboardStatsDTO> getSupervisorStats(@AuthenticationPrincipal UserDetails userDetails) {
        User supervisor = authService.getUserByEmail(userDetails.getUsername());
        return ResponseEntity.ok(adminService.getSupervisorDashboardStats(supervisor));
    }
}
