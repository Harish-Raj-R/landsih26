  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (from && from !== '/' && !from.includes('/login')) {
        ...
        // After successful login, proceed to the 2nd step (Map Page) or role dashboard
        navigate('/india-map', { replace: true });
      }
    } catch (err) {
      setError(
              ...
              {mode === 'citizen' ? (
                ...
                <div className="flex flex-wrap items-center justify-center gap-1.5">
                  <button
                    ...
                    className="px-2.5 py-1 bg-slate-100 hover:bg-purple-50 text-slate-700 hover:text-purple-700 text-[11px] rounded-md font-mono border border-slate-200 transition cursor-pointer"
                  >
                    {t('role_supervisor') || 'Supervisor'}
                  </button>
                  <button
                    type="button"
                    onClick={() => handleQuickFill('officer@landstack.demo')}
                    className="px-2.5 py-1 bg-slate-100 hover:bg-teal-50 text-slate-700 hover:text-teal-700 text-[11px] rounded-md font-mono border border-slate-200 transition cursor-pointer"
                  >
                    {t('role_field_officer') || 'Field Officer'}
                  </button>
                  <button
                    type="button"
                    onClick={() => handleQuickFill('admin@landstack.demo')}
                    className="px-2.5 py-1 bg-slate-100 hover:bg-rose-50 text-slate-700 hover:text-rose-700 text-[11px] rounded-md font-mono border border-slate-200 transition cursor-pointer"
                  >
                    {t('role_admin') || 'Admin'}
                  </button>
                </div>
